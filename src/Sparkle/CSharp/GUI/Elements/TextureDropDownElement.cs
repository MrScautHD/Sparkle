...
public class TextureDropDownElement : GuiElement {
    ...
    private void DrawNormal(GuiRenderQueue renderQueue, int localOrder, Texture2D texture, Sampler? sampler, Rectangle sourceRect, Color color, SpriteFlip flip, bool pixelSnap, Effect? effect = null, BlendStateDescription? blendState = null, DepthStencilStateDescription? depthStencilState = null) {
        ...
        }, (Texture: texture, Position: position, SourceRect: sourceRect, Scale: scale, Origin: origin, PixelSnap: pixelSnap, Rotation: rotation, Color: color, Flip: flip), renderState);
    }
    ...
    private void DrawNineSlice(GuiRenderQueue renderQueue, int localOrder, Texture2D texture, Sampler? sampler, Rectangle sourceRect, BorderInsets borderInsets, bool tileCenter, Color color, SpriteFlip flip, bool pixelSnap, Effect? effect = null, BlendStateDescription? blendState = null, DepthStencilStateDescription? depthStencilState = null) {
        SpriteGuiRenderState renderState = new SpriteGuiRenderState(sampler, effect, blendState, depthStencilState);
        Vector2 elementPosition = this.Position;
        Vector2 elementOrigin = this.Origin;
        Vector2 elementSize = this.Size;
        Vector2 elementScale = this.Scale;
        int guiScaleFactor = this.Gui.ScaleFactor;
        float elementRotation = this.Rotation;
        ...
        renderQueue.SubmitSprite(localOrder, static (batch, state) => {
            ...
            Vector2 position = pixelSnap ? Vector2.Floor(state.ElementPosition) : state.ElementPosition;
            Vector2 origin = pixelSnap ? Vector2.Floor(state.ElementOrigin) : state.ElementOrigin;
            Vector2 size = pixelSnap ? Vector2.Floor(state.ElementSize) : state.ElementSize;
            Vector2 scale = pixelSnap ? Vector2.Max(Vector2.One, Vector2.Floor(state.ElementScale)) * state.GuiScaleFactor : state.ElementScale * state.GuiScaleFactor;
            ...
            // Draw Corners.
            batch.DrawTexture(texture, position, 0.5F, sourceTopLeft, scale, pivot / scale, false, state.ElementRotation, color, flip);
            batch.DrawTexture(texture, position, 0.5F, sourceTopRight, scale, (pivot - new Vector2(finalSize.X - rightW, 0.0F)) / scale, false, state.ElementRotation, color, flip);
            batch.DrawTexture(texture, position, 0.5F, sourceBottomLeft, scale, (pivot - new Vector2(0.0F, finalSize.Y - bottomH)) / scale, false, state.ElementRotation, color, flip);
            batch.DrawTexture(texture, position, 0.5F, sourceBottomRight, scale, (pivot - new Vector2(finalSize.X - rightW, finalSize.Y - bottomH)) / scale, false, state.ElementRotation, color, flip);
            ...
            if (innerH > 0.0F) {
                if (tileCenter) {
                    ...
                    for (float y = 0.0F; y < innerH; y += tileH) {
                        ...
                        batch.DrawTexture(texture, position, 0.5F, cL, scale, (pivot - new Vector2(0.0F, topH + y)) / scale, false, state.ElementRotation, color, flip);
                        batch.DrawTexture(texture, position, 0.5F, cR, scale, (pivot - new Vector2(finalSize.X - rightW, topH + y)) / scale, false, state.ElementRotation, color, flip);
                    }
                }
                else {
                    Vector2 sV = new Vector2(scale.X, innerH / sourceLeft.Height);
                    batch.DrawTexture(texture, position, 0.5F, sourceLeft, sV, (pivot - new Vector2(0.0F, topH)) / sV, false, state.ElementRotation, color, flip);
                    batch.DrawTexture(texture, position, 0.5F, sourceRight, sV, (pivot - new Vector2(finalSize.X - rightW, topH)) / sV, false, state.ElementRotation, color, flip);
                }
            }
            ...
            if (innerW > 0.0F) {
                if (tileCenter) {
                    ...
                    for (float x = 0.0F; x < innerW; x += tileW) {
                        ...
                        batch.DrawTexture(texture, position, 0.5F, cT, scale, (pivot - new Vector2(leftW + x, 0.0F)) / scale, false, state.ElementRotation, color, flip);
                        batch.DrawTexture(texture, position, 0.5F, cB, scale, (pivot - new Vector2(leftW + x, finalSize.Y - bottomH)) / scale, false, state.ElementRotation, color, flip);
                    }
                }
                else {
                    ...
                    batch.DrawTexture(texture, position, 0.5F, cT, sH, (pivot - new Vector2(leftW, 0.0F)) / sH, false, state.ElementRotation, color, flip);
                    batch.DrawTexture(texture, position, 0.5F, cB, sH, (pivot - new Vector2(leftW, finalSize.Y - bottomH)) / sH, false, state.ElementRotation, color, flip);
                }
            }
                ...
                if (tileCenter) {
                    ...
                    for (float y = 0.0F; y < innerH; y += tileH) {
                        ...
                        for (float x = 0.0F; x < innerW; x += tileW) {
                            ...
                            batch.DrawTexture(texture, position, 0.5F, cC, scale, (pivot - new Vector2(leftW + x, topH + y)) / scale, false, state.ElementRotation, color, flip);
                        }
                    }
                }
                else {
                    ...
                    batch.DrawTexture(texture, position, 0.5F, cC, sC, (pivot - new Vector2(leftW, topH)) / sC, false, state.ElementRotation, color, flip);
                }
            }
        }, (Texture: texture, ElementPosition: elementPosition, SourceRect: sourceRect, ElementOrigin: elementOrigin, ElementSize: elementSize, ElementScale: elementScale, GuiScaleFactor: guiScaleFactor, ElementRotation: elementRotation, BorderInsets: borderInsets, TileCenter: tileCenter, Color: color, Flip: flip, PixelSnap: pixelSnap), renderState);
    }
    ...
    private void DrawMenu(GuiRenderQueue renderQueue, Color color, DepthStencilStateDescription? depthStencilState = null) {
        ...
        int localOrder = depthStencilState.HasValue ? 6 : 2;
        
        switch (this.DropDownData.MenuResizeMode) {
            case ResizeMode.None:
                this.DrawNormal(renderQueue, localOrder, this.DropDownData.MenuTexture, this.DropDownData.MenuSampler, this.DropDownData.MenuSourceRect, color, this.DropDownData.MenuFlip, this.DropDownData.MenuPixelSnap, this.DropDownData.Effect, this.DropDownData.BlendState, depthStencilState);
                break;
            
            case ResizeMode.NineSlice:
            case ResizeMode.TileCenter:
                this.DrawNineSlice(renderQueue, localOrder, this.DropDownData.MenuTexture, this.DropDownData.MenuSampler, this.DropDownData.MenuSourceRect, this.DropDownData.MenuBorderInsets, this.DropDownData.MenuResizeMode == ResizeMode.TileCenter, color, this.DropDownData.MenuFlip, this.DropDownData.MenuPixelSnap, this.DropDownData.Effect, this.DropDownData.BlendState, depthStencilState);
                break;
        }
    ...
    private void DrawHighlight(GuiRenderQueue renderQueue) {
        ...
        renderQueue.SubmitPrimitive(4, static (batch, state) => {
            batch.DrawFilledRectangle(state.Rectangle, state.Origin, state.Rotation, 0.5F, state.Color);
        }, (Rectangle: highlightMaskRect, Origin: highlightMaskOrigin, Rotation: this.Rotation, Color: new Color(255, 255, 255, 0)), primitiveReadMask0RenderState);
    ...
    private void DrawSliderBar(GuiRenderQueue renderQueue) {
        ...
        switch (this.DropDownData.FieldResizeMode) {
            case ResizeMode.None:
                this.DrawNormal(renderQueue, 3, this.DropDownData.SliderBarTexture, this.DropDownData.SliderBarSampler, this.DropDownData.SliderBarSourceRect, color, this.DropDownData.SliderBarFlip, this.DropDownData.SliderBarPixelSnap, this.DropDownData.Effect, this.DropDownData.BlendState);
                break;
            
            case ResizeMode.NineSlice:
            case ResizeMode.TileCenter:
                this.DrawNineSlice(renderQueue, 3, this.DropDownData.SliderBarTexture, this.DropDownData.SliderBarSampler, this.DropDownData.SliderBarSourceRect, this.DropDownData.SliderBarBorderInsets, this.DropDownData.SliderBarResizeMode == ResizeMode.TileCenter, color, this.DropDownData.SliderBarFlip, this.DropDownData.SliderBarPixelSnap, this.DropDownData.Effect, this.DropDownData.BlendState);
                break;
        }
    ...
    private void DrawSlider(GuiRenderQueue renderQueue) {
        ...
        renderQueue.SubmitSprite(4, static (batch, state) => {
        ...
        }, (Texture: this.DropDownData.SliderTexture, Position: this.Position, Depth: 0.5F, SourceRect: this.DropDownData.SliderSourceRect, Scale: this.Scale * this.Gui.ScaleFactor, Origin: origin, PixelSnap: this.DropDownData.SliderPixelSnap, Rotation: this.Rotation, Color: color, Flip: this.DropDownData.SliderFlip), renderState);
    }
    ...
    private void DrawArrow(GuiRenderQueue renderQueue) {
        ...
        renderQueue.SubmitSprite(8, static (batch, state) => {
            batch.DrawTexture(state.Texture, state.Position, state.Depth, state.SourceRect, state.Scale, state.Origin, state.PixelSnap, state.Rotation, state.Color, state.Flip);
        }, (Texture: this.DropDownData.ArrowTexture, Position: arrowWorldCenter, Depth: 0.5F, SourceRect: this.DropDownData.ArrowSourceRect, Scale: this.Scale * this.Gui.ScaleFactor, Origin: arrowCenter, PixelSnap: this.DropDownData.ArrowPixelSnap, Rotation: this.Rotation + float.RadiansToDegrees(this._arrowRotation), Color: color, Flip: this.DropDownData.ArrowFlip), renderState);
    }
    ...
    private void DrawText(GuiRenderQueue renderQueue, LabelData labelData, TextAlignment textAlignment, Vector2 textOffset, Vector2 textScale, DepthStencilStateDescription? depthStencilState = null) {
        ...
        renderQueue.SubmitSprite(depthStencilState.HasValue ? 7 : 1, static (batch, state) => {
            batch.DrawText(
