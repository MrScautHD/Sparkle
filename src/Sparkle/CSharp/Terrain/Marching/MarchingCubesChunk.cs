...
public class MarchingCubesChunk : Disposable, IChunk {
    ...
    /// <summary>
    /// The uploaded GPU mesh, or <c>null</c> when the chunk has no geometry or is using region batching.
    /// </summary>
    ...
    /// <summary>
    /// CPU-side vertex buffer populated by <see cref="GenerateGeometry"/> and consumed by
    /// <see cref="UploadGeometry"/> or the region batch system via <see cref="GetPendingVertices"/>.
    /// </summary>
    private List<Vertex3D> _vertices;

    /// <summary>
    /// CPU-side index buffer populated by <see cref="GenerateGeometry"/> and consumed by
    /// <see cref="UploadGeometry"/> or the region batch system via <see cref="GetPendingIndices"/>.
    /// </summary>
    ...
    /// <summary>
    /// Returns a copy of the CPU-side vertices produced by the most recent <see cref="GenerateGeometry"/> call,
    /// in chunk-local space. Returns an empty array when the chunk produced no geometry.
    /// Call before <see cref="UploadGeometry"/> or <see cref="DiscardGeometry"/>, as both clear this data.
    /// </summary>
    public Vertex3D[] GetPendingVertices() => this._vertices.ToArray();

    /// <summary>
    /// Returns a copy of the CPU-side indices produced by the most recent <see cref="GenerateGeometry"/> call.
    /// Returns an empty array when the chunk produced no geometry.
    /// Call before <see cref="UploadGeometry"/> or <see cref="DiscardGeometry"/>, as both clear this data.
    /// </summary>
    public uint[] GetPendingIndices() => this._indices.ToArray();

    /// <summary>
    /// Disposes the individual GPU mesh (if any) and clears the CPU vertex/index buffers without
    /// uploading to the GPU. Use this instead of <see cref="UploadGeometry"/> when the chunk's
    /// geometry has been consumed by a region batch merge operation.
    /// </summary>
    ...
    /// <summary>
    /// Uploads the geometry prepared by <see cref="GenerateGeometry"/> to the GPU as an individual chunk mesh.
    /// Not used when region batching is active — prefer <see cref="GetPendingVertices"/> +
    /// <see cref="GetPendingIndices"/> + <see cref="DiscardGeometry"/> in that case.
    /// </summary>
    ...
    /// <summary>
    /// Returns <c>true</c> when the chunk is entirely solid or entirely empty.
    /// </summary>
    ...
    private void ComputeSmoothNormals() {
        ...
        for (int i = 0; i < vertexCount; i++) {
            ...
            vertex.Normal      = this._normalAccumulator[i].LengthSquared() > float.Epsilon
                ? -Vector3.Normalize(this._normalAccumulator[i])
                : Vector3.UnitY;
    ...
    private Vector3 GetEdgeVertex(Vector3 cellOrigin, int edgeIndex, float[] cornerDensities, int stepX, int stepY, int stepZ) {
        ...
        float densityDelta = MathF.Abs(delta) <= float.Epsilon
            ? 0.5F
            : Math.Clamp((this.Terrain.IsoLevel - density1) / delta, 0.0F, 1.0F);
